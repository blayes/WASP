% dirp = '/Shared/ssrivastva/wasp/ml/result/wasp/'
function calc_wasp_cov_2d_k10(dd, nsub, dirp)
...
for dims = 1:ndim
    covs = {};
    for jj = 1:nsub
        covs{jj} = csvread(strcat(dirp, 'samp/joint/cov_cv_', num2str(dd), ...
                                   '_nsub_', num2str(jj), '_d_', num2str(dims), '_k10.csv'));         
    end
    
    subsetPost = {};    
    for jj = 1:nsub
        subsetPost{jj} = covs{jj}(randi([1 1000], 100, 1), :);
    end         
    ...
    save(strcat(dirp, 'joint/joint_cov_cv_',  num2str(dd), '_d_', num2str(dims), '_k10.mat'), 'tmats','avec', 'subsetPost', 'overallPost');    
    summ = [overallPost avec];
    csvwrite(strcat(dirp, 'joint/wasp_cov_cv_',  num2str(dd), '_d_', num2str(dims), '_k10.csv'), summ);
    
    disp(['done with dim ' num2str(dims)]);    
end    

csvwrite(strcat(dirp, 'joint/cov_2d_times_cv_', num2str(dd),  '_k10.csv'), rtime);    
